...
def build_features(asset):
    try:
        ...
        df["DXY"], df["VIX"] = macro["DXY"], macro["VIX"]
...
def train_predict(df, horizon):
    ...
    pred = df["Ret1"].tail(60).mean() if "Ret1" in df else 0
    return pred, {"model":"heuristic","n":n}
...
def main():
    ...
    macro = fetch_macro_timeseries()
    if not macro.empty:
        last = macro.tail(1).iloc[0]
        st.write(f"DXY: {last['DXY']:.2f} | VIX: {last['VIX']:.2f}")
    else:
