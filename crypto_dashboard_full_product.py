...
Styling-Änderung: EMA-Linien sind jetzt durchgezogen (keine gestrichelten Linien).
"""
        ...
        for ticker in WATCHLIST:
            ...
            for p in (20,50,200):
                if f"EMA{p}" in df.columns:
                    # EMA lines: solid (durchgezogen)
                    fig.add_trace(go.Scatter(x=df.index, y=df[f"EMA{p}"], name=f"EMA{p}", line=dict(width=1)))
            fig.update_layout(height=420)
            st.plotly_chart(fig, use_container_width=True)
