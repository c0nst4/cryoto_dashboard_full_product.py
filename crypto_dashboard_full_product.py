...
# ---------------- MULTI-TIMEFRAME MACHINE LEARNING MODEL ----------------
def build_multi_tf_features(asset):
    """
    Baut kombinierte technische Features aus 4h, 1D und 1W Timeframes.
    Gibt zusammengeführtes DataFrame zurück.
    """
    tf_settings = {"4h": "4h", "1D": "1d", "1W": "1wk"}
    dfs = []

    for tf_label, tf_interval in tf_settings.items():
        try:
            df = yf.download(asset, period="1y", interval=tf_interval, progress=False)
            if df is None or df.empty:
                continue
            df = compute_technical(df)
            df = df.rename(columns=lambda x: f"{x}_{tf_label}")
            dfs.append(df)
        except Exception:
            continue
    ...
    # Zusammenführen auf Basis von Datum (Index)
    df_merged = dfs[0]
    for other in dfs[1:]:
        df_merged = df_merged.join(other, how="outer", rsuffix="_dup")

    # Fehlende Werte auffüllen
    df_merged = df_merged.fillna(method="ffill").fillna(method="bfill").fillna(0)

    # Füge Makrodaten hinzu (DXY, VIX)
    macro = fetch_macro_timeseries()
    if not macro.empty:
        macro = macro.reindex(df_merged.index).fillna(method="ffill").fillna(0)
        df_merged["DXY"] = macro["DXY"]
        df_merged["VIX"] = macro["VIX"]
    else:
        df_merged["DXY"] = 0
        df_merged["VIX"] = 0
    ...
    # Fear & Greed Index
    fgi, lbl = fetch_fear_greed()
    df_merged["FearGreed"] = fgi if fgi else 50
...
def train_predict_multi_tf(asset, horizon_days=7):
    """
    Trainiert ein kombiniertes ML-Modell auf 4h + 1D + 1W Indikatoren.
    Nutzt Gradient Boosting oder Linear Regression.
    """
    df = build_multi_tf_features(asset)
    if df.empty:
        return None, {"status": "no_data"}

    # Zielvariable: Preisänderung in % über den Prognosehorizont
    close_cols = [c for c in df.columns if "Close_" in c]
    if not close_cols:
        return None, {"status": "no_close"}
    close = df[close_cols[0]]
    df["target"] = close.shift(-horizon_days) / close - 1
    df = df.dropna()
    n = len(df)
    if n < 40:
        return None, {"status": "not_enough_rows", "n": n}

    # Features: alle RSI/MACD/EMA/SMA aus allen TFs + Makro
    features = [c for c in df.columns if any(x in c for x in ["RSI", "MACD", "EMA", "SMA", "Vol", "DXY", "VIX", "FearGreed"])]
    if not features:
        return None, {"status": "no_features"}
    ...
    try:
        ...
        r2 = float(r2_score(yte, model.predict(Xte)))
        return pred, {"model": "GB-MTF", "r2": r2, "n": n}
    except Exception:
        # Fallback: lineare Regression
        try:
            from sklearn.linear_model import LinearRegression
            lr = LinearRegression().fit(X, y)
            pred = float(lr.predict(df[features].tail(1).values)[0])
            return pred, {"model": "LR-MTF", "n": n}
        except Exception:
